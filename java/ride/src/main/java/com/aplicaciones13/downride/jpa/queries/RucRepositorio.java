...
public interface RucRepositorio extends JpaRepository<Ruc, Integer> {
     ...
     */
    @Query(value = "SELECT id_rucs, ruc, contrasena, estado, usuario_fecha, usuario_programa, fecha_inicio FROM public.rucs WHERE estado = 'A'", nativeQuery = true)
    List<Ruc> findAll();

    /**
     * Metodo para buscar por id_rucs
     */
    @Query(value = "SELECT id_rucs, ruc, contrasena, estado, usuario_fecha, usuario_programa, fecha_inicio FROM public.rucs WHERE id_rucs = ?1 AND estado = 'A'", nativeQuery = true)
    Ruc findByIdRucs(Integer idRucs);

    /**
     * Metodo para buscar por ruc
     */
    @Query(value = "SELECT id_rucs, ruc, contrasena, estado, usuario_fecha, usuario_programa, fecha_inicio FROM public.rucs WHERE ruc = ?1 AND estado = 'A'", nativeQuery = true)
     ...
     * @param fechaInicio
     */
    @Query(value = "UPDATE public.rucs SET fecha_inicio = ?2 WHERE id_rucs = ?1", nativeQuery = true)
    void updateFechaInicio(Integer idRucs, String fechaInicio);
